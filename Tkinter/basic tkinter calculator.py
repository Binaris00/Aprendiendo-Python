#Importar todo el modulo/libreria de tkinter
from tkinter import *

root = Tk() #Crear la ventana
root.title("Basic Calculator") #Ponerle un titulo a la ventana
...
def show(value):
    global equation
    equation+=value
    label_result_calculator.config(text=equation)
...
def calculate():
    ...
    if equation !="":
        try:
            ...
            result="error"
            quation = ""
    label_result_calculator.config(text=result)

label_result_calculator = Label(root, width=25, height=2, text="",font=("Arial", 30))
label_result_calculator.pack()
